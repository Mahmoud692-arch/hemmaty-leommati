...
export const Route = createFileRoute("/hadiths/muslim")({
  head: () => ({
    meta: [
      { title: "صحيح مسلم — هِمَّتي لِأمّتي" },
      {
        name: "description",
        content: "أحاديث من صحيح الإمام مسلم رحمه الله، مخرج ورواة وشرح الأحاديث لعامة المسلمين.",
      },
    ],
  }),
  component: MuslimHadithsPage,
});
  ...
  useEffect(() => {
    const fetchHadiths = async () => {
      ...
      try {
        ...
        console.error("Error fetching Muslim hadiths:", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="container mx-auto px-4 py-12 md:py-16 max-w-5xl">
      ...
      <div className="text-center mb-10">
        <h1 className="font-display text-4xl md:text-5xl mb-3">صحيح الإمام مسلم</h1>
        <p className="text-muted-foreground max-w-2xl mx-auto">
          تصفح أحاديث صحيح مسلم، المصنف الثاني في الصحة في الحديث النبوي الشريف، مع شرح ميسر وفوائد.
        </p>
        <div className="inline-flex items-center gap-2 mt-4 text-xs px-3 py-1.5 rounded-full bg-[var(--gold)]/15 text-[var(--gold-foreground)] dark:text-[var(--gold)]">
          <ShieldCheck className="h-3.5 w-3.5" /> أحاديثُ صحيحة مسندة وموثقة
        </div>
        <OrnamentalDivider />
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-12 text-muted-foreground">
          لا توجد أحاديث مطابقة لبحثك في صحيح مسلم.
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-10">
            {hadiths.map((h) => (
              <Link
                ...
                params={{ collection: "muslim", number: String(h.number) }}
              ...
              >
                <div>
                  <div className="flex items-start justify-between gap-3 mb-3">
                    ...
                    <span className="text-[10px] px-2 py-0.5 rounded-full bg-blue-500/10 text-blue-600 dark:text-blue-500 font-medium">
                      صحيح مسلم
                    </span>
                  </div>
